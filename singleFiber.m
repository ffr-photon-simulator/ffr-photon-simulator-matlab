function []=singleFiber(nfil, nfiw, latticeOffset, incShift, stepFactor)
% CONSTANTS
...
for row = 1:size(fiber_lattice)
  thisfiberXCoord = fiber_lattice(row,1);
  thisfiberYCoord = fiber_lattice(row,2);
  thisfiberCoords = [thisfiberXCoord thisfiberYCoord];
  plotFiberCircle(thisfiberCoords, fiber_reflection_radius)
end
% Photons (in blue)
...
function [newXStep, newYStep] = calculateNewSteps(photonCoords, previousPhotonCoords, reflectedFiberCoords, general_photon_step)
  nextPoint = calculateReflectedPathPoint(photonCoords, reflectedFiberCoords, previousPhotonCoords);
  reflectedPathSlope = calculateSlope(nextPoint, photonCoords);
  scaleFactor = sqrt(reflectedPathSlope^2 + 1) / general_photon_step; % hypotenuse scaled down to general step
  newXStep = 1 / scaleFactor;
  newYStep = reflectedPathSlope / scaleFactor;
  if (nextPoint(1) < reflectedFiberCoords(1))
      newXStep = newXStep * -1;
  end
 if (nextPoint(2) < reflectedFiberCoords(2))
     newYStep = newYStep * -1;
  end
end

function newPoint = calculateReflectedPathPoint(photonCoords, reflectedFiberCoords, previousPhotonCoords)
  incidentSlope = calculateSlope(photonCoords, reflectedFiberCoords);
  yIntercept = calculateYIntercept(photonCoords, reflectedFiberCoords);
  a = 1 + incidentSlope^2;
  b = 1 - incidentSlope^2;
  xCoord = (b/a)*previousPhotonCoords(1) + ((2*incidentSlope)/a)*(previousPhotonCoords(2) - yIntercept);
  yCoord = (b/a)*(previousPhotonCoords(2) - yIntercept) + (2*incidentSlope*previousPhotonCoords(1))/a + yIntercept;
...
%    else
      ...
      %stepAngle = calculateSlopeAngle(reflectionPoint, reflectedFiberCoords);
%      xDistance = abs(reflectionPoint(1) - reflectedFiberCoords(1));
%      yDistance = abs(reflectionPoint(2) - reflectedFiberCoords(2));
%      stepAngle = atan(xDistance/yDistance); % x/y is particular to this geometry
...
%  else
    %disp("Slope not inf.")
%    radiusSlopeAngle = calculateSlopeAngle(reflectionPoint, reflectedFiberCoords);
%    yIntercept = calculateYIntercept(radiusSlopeAngle, reflectedFiberCoords);
    %disp("Y int point: " + coordToString(yIntercept))
    %plot(yIntercept(1),yIntercept(2),'k.','MarkerSize',10);
%    reflectedPoint = reflectPoint(previousPhotonCoords, radiusSlopeAngle, yIntercept);
    %disp("Reflected point: " + coordToString(reflectedPoint))
    %plot(reflectedPoint(1),reflectedPoint(2),'k.','MarkerSize',10);
%    stepAngle = calculateSlopeAngle(reflectedPoint, reflectionPoint);
%    newXStep = general_photon_step * sin(stepAngle);
%    newYStep = general_photon_step * cos(stepAngle);
%  end
%end

function angle = calculateSlopeAngle(point1, point2)
  xDistance = point1(1) - point2(1);
  yDistance = point1(2) - point2(2);
  angle = atan(yDistance/xDistance);
end

function yIntercept = calculateYIntercept(point1, point2)
  % (y-y1) = m(x-x1)
  % b = y1 - m(x1)
  slope = calculateSlope(point1, point2);
  b = point1(2) - (slope * point1(1));
  yIntercept = [0 b];
end
...
function reflectedPoint = reflectPoint(point, angle, yIntercept)
  ...
  doubleAngle = 2*angle;
  reflectionMatrix = [cos(doubleAngle) sin(doubleAngle); sin(doubleAngle) -cos(doubleAngle) ];
...
function slope = calculateSlope(point1, point2)
...
function [reflected, reflectedFiberCoords] = checkIfReflected(photonCoords,fiber_lattice,fiber_reflection_radius)
  ...
  %for latticeRow = 1:number_fibers_in_width
    for row = 1:size(fiber_lattice)
        ...
        thisfiberYCoord = fiber_lattice(row,2);
        thisfiberCoords = [thisfiberXCoord thisfiberYCoord];
        photonDistanceToFiber = calculatePhotonDistanceToFiber(photonCoords, thisfiberCoords);
        if photonDistanceToFiber < fiber_reflection_radius
          %disp("  Photon distance to fiber: " + photonDistanceToFiber)
          reflected = true;
          reflectedFiberCoords = thisfiberCoords;
        return;
        end
    end
  reflected = false;
