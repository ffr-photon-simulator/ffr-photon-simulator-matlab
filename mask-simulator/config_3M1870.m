% Configuration file to simulate ray tracing with a 3M 1870+ FFR.

% The FFR config, later renamed to ffrConfig.
c = struct();
c.nLayers = Config.ffr_3M1870.nLayers; % number of FFR layers
c.model = Config.ffr_3M1870.model;
% Length
c.lengthI = Config.ffr_3M1870.lengthI;
...
c.layerRadiiI = Config.ffr_3M1870.layerRadiiI;
...
c.layerDensitiesI = Config.ffr_3M1870.layerDensitiesI;

% FFR layer widths.
% 0.9mm thick with 100μ FFR layer thickness
c.layerWidthsI = Config.ffr_3M1870.layerWidthsI; % list, not 9-row matrix
...
xStart = Config.ffr_3M1870.xStart;
xEnd = Config.ffr_3M1870.xEnd;
...
for i = 1:c.nLayers
  ...
  nQLayers = width / Config.ffr_3M1870.qWidth; % qLayers are same width as quadrants
  % We use this to estimate the total number of fibers during array pre-allocation.
  nQuadrantsPerQLayer = c.length / Config.ffr_3M1870.qLength;
  ...
  for j = 1:l.nQLayers
    ql = Config.buildQuadrantLayerConfig(c, outerBoundHeight, Config.ffr_3M1870.qLength, Config.ffr_3M1870.qWidth, nQuadrantsPerQLayer);
    ...
    for k = 1:ql.nQuadrants
      q = Config.buildQuadrantConfig(c, Config.ffr_3M1870.qLength, Config.ffr_3M1870.qWidth, ...
